...
function getRandomOffset(baseMinutes, minOffset, maxOffset) {
...
function getNextIntervalMinutes() {
  ...
  if (hour >= config.dayStartHour && hour < config.dayEndHour) {
    return getRandomOffset(config.dayIntervalMinutes, 3, 5);
  }
  return getRandomOffset(config.nightIntervalMinutes, 5, 10);
}
...
async function scheduleNextCheck() {
  ...
  const intervalMinutes = getNextIntervalMinutes();
  const intervalMs = intervalMinutes * 60 * 1000;
  
  log(`下次检查将在 ${intervalMinutes.toFixed(1)} 分钟后进行`);
...
function scheduleRecoveryCheck() {
  ...
  log(`熔断恢复: 将在 ${COOLDOWN_MINUTES} 分钟后进行探测检查`);
...
async function sendAlert(message) {
  ...
  if (config.pushPlusToken) {
    try {
      ...
      log(`PushPlus告警推送失败: ${error.message}`);
    }
  }
  ...
  if (config.dingTalkToken) {
    try {
      ...
      log(`钉钉告警推送失败: ${error.message}`);
    }
  }
  
  if (config.barkUrl) {
    try {
      await axios.get(`${config.barkUrl}/成绩查询脚本告警/${encodeURIComponent(message)}`);
      log('Bark告警推送成功');
    } catch (error) {
      log(`Bark告警推送失败: ${error.message}`);
    }
  }
}
...
function triggerCircuitBreaker(reason) {
  isPaused = true;
  log(`⚠️ 触发熔断机制，脚本暂停: ${reason}`);
  sendAlert(`脚本已暂停：连续${FAILURE_THRESHOLD}次${reason}，请手动检查账号状态。${COOLDOWN_MINUTES}分钟后将自动尝试恢复。`);
...
async function getGrades() {
  ...
  try {
    ...
    consecutiveFailures++;
    log(`连续失败次数: ${consecutiveFailures}/${FAILURE_THRESHOLD}`);
    
    if (consecutiveFailures >= FAILURE_THRESHOLD) {
      triggerCircuitBreaker('登录失败');
    }
...
async function sendPushPlus(message) {
  ...
  try {
    ...
    log(`PushPlus推送失败: ${error.message}`);
  }
}
...
async function sendBark(message) {
  if (!config.barkUrl) return;
  
  try {
    await axios.get(`${config.barkUrl}/成绩更新提醒/${encodeURIComponent(message)}`);
    log('Bark推送成功');
  } catch (error) {
    log(`Bark推送失败: ${error.message}`);
  }
}
...
async function sendNotification(changes) {
  ...
  if (message) {
    ...
    await sendPushPlus(message);
    await sendDingTalk(message);
    await sendBark(message);
  }
}

async function runCheck(isRecovery = false) {
  if (isRunning) {
    log('检查正在进行中，跳过本次');
    if (!isRecovery) {
      scheduleNextCheck();
  ...
  try {
    ...
    if (!grades || grades.length === 0) {
      log('未查询到成绩');
      if (!isRecovery && !isPaused) {
        scheduleNextCheck();
      }
      return;
    }
    
    const isFirstRun = !fs.existsSync(CACHE_FILE) || cachedGrades.length === 0;
...
process.on('uncaughtException', (error) => {
  log(`⚠️ 未捕获异常: ${error.message}`);
  log(`异常堆栈: ${error.stack}`);
  sendAlert(`脚本发生未捕获异常: ${error.message}`);
...
process.on('unhandledRejection', (reason, promise) => {
  log(`⚠️ 未处理的Promise拒绝: ${reason}`);
  sendAlert(`脚本发生Promise拒绝: ${reason}`);
  if (!isPaused && !isRunning) {
    log('重新调度下一次检查...');
    scheduleNextCheck();
  }
...
async function main() {
  ...
  log('成绩查询脚本启动...');
  log(`安全设置: 连续${FAILURE_THRESHOLD}次失败将触发熔断`);
  log(`熔断恢复: 暂停${COOLDOWN_MINUTES}分钟后自动尝试恢复`);
  log(`白天(${config.dayStartHour}:00-${config.dayEndHour}:00): 每${config.dayIntervalMinutes}±5分钟检查一次`);
  log(`晚上(${config.dayEndHour}:00-${config.dayStartHour}:00): 每${config.nightIntervalMinutes}±10分钟检查一次`);
